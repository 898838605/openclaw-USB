...
import { d as normalizeSecretInputString } from "./types.secrets-CJS3n8Im.js";
import { r as normalizeProviderId } from "./provider-id-Cz7K6wgK.js";
import { n as defaultRuntime } from "./runtime-DDH_zqCr.js";
import { r as createConfigIO } from "./io-DnGGugrj.js";
...
import { n as t } from "./i18n-G-rDmJiH.js";
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-S29B5mY4.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-BMyTKZr6.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-D9OdpAbl.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-De_oXv4a.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-lfJREcC8.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CbbQ2CSd.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-8ZR-kWih.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-cXNgcAvq.js"), import("./setup-registry-IEtiedb5.js")]);
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-D6pCbldd.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-B2yumqeY.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-BDwxj0k0.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-BDwxj0k0.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-smJcWocD.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-DKmaBH-H.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DdxyAO54.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-048uGBh8.js");
		const { setupChannels } = await import("./onboard-channels-DvvGYfDH.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-DVmhM14G.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-DwXp9tE1.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-D80e3fQ7.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-BeVl2G-F.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-QMHD3vPV.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DdxYEL5V.js");
