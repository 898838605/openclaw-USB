...
import { t as addGatewayServiceCommands } from "./register-service-commands-CdnztxuH.js";
import { n as defaultRuntime } from "./runtime-DDH_zqCr.js";
import { t as formatHelpExamples } from "./help-format-B0QfO_8n.js";
import { t as addGatewayRunCommand } from "./run-command-ULopF727.js";
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-CapGDpX7.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-CO9qHOtq.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(() => import("./bonjour-discovery-BRMS0WO2.js"));
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-DDLbfwX0.js"));
const healthStyleModuleLoader = createLazyImportLoader(() => import("./health-style-CX2jL7bb.js"));
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-DAebqpXt.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-BS9EKC5M.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-DUQ2IK3h.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-DWIliaQc.js"));
...
async function callGatewayCli(method, opts, params) {
	return (await import("./call-d2Slae7M.js")).callGatewayCli(method, opts, params);
}
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayTransportErrorJson } = await import("./call-D5BLgL4L.js");
...
function registerGatewayCli(program) {
	...
	gatewayCallOpts(gateway.command("stability").description("Fetch payload-free Gateway stability diagnostics").option("--limit <limit>", "Maximum number of recent events", "25").option("--type <type>", "Filter by diagnostic event type").option("--since-seq <seq>", "Only include events after this sequence").option("--bundle [path]", "Read a persisted stability bundle instead of calling Gateway; pass \"latest\" for newest").option("--export", "Write a shareable support diagnostics export", false).option("--output <path>", "Diagnostics export output .zip path").action(async (opts, command) => {
		await runGatewayCommand(async () => {
			const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } = await import("./diagnostic-stability-Bjm9NBg0.js");
			const rpcOpts = resolveGatewayRpcOptions(opts, command);
	...
	gateway.command("discover").description("Discover gateways via Bonjour (local + wide-area if configured)").option("--timeout <ms>", "Per-command timeout in ms", "2000").option("--json", "Output JSON", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			const [{ readSourceConfigBestEffort }, { discoverGatewayBeacons }, { resolveWideAreaDiscoveryDomain }, { dedupeBeacons, parseDiscoverTimeoutMs, pickBeaconHost, pickGatewayPort, renderBeaconLines }, { withProgress }] = await Promise.all([
				...
				import("./discover-DoVKQjND.js"),
				import("./progress-Cg31UjtD.js")
			]);
