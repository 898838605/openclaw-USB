import { l as onAgentEvent, t as clearAgentRunContext } from "./agent-events-DVSiKwui.js";
import { n as onSessionTranscriptUpdate } from "./transcript-events-Pp-VxfGB.js";
import { n as onSessionLifecycleEvent } from "./session-lifecycle-events-h4GP6omb.js";
import { r as onHeartbeatEvent } from "./heartbeat-events-Bz-N9TLi.js";
...
function startGatewayEventSubscriptions(params) {
	...
	const getAgentEventHandler = () => {
		agentEventHandlerPromise ??= Promise.all([import("./server-chat-Bvm45tyg.js"), import("./server-session-key-D-T0mHul.js")]).then(([{ createAgentEventHandler }, { resolveSessionKeyForRun }]) => createAgentEventHandler({
	...
	const getTranscriptUpdateHandler = () => {
		transcriptUpdateHandlerPromise ??= import("./server-session-events-DsRVrzsp.js").then(({ createTranscriptUpdateBroadcastHandler }) => createTranscriptUpdateBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers,
	...
	const getLifecycleEventHandler = () => {
		lifecycleEventHandlerPromise ??= import("./server-session-events-DsRVrzsp.js").then(({ createLifecycleEventBroadcastHandler }) => createLifecycleEventBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers
		}));
