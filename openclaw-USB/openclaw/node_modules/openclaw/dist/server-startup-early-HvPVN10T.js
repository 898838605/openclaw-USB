...
async function startGatewayPluginDiscovery(params) {
	...
	const machineDisplayName = await measureStartup(params.startupTrace, "runtime.early.discovery.machine-name", async () => (await import("./machine-name-BIbim3hP.js")).getMachineDisplayName());
	return await measureStartup(params.startupTrace, "runtime.early.discovery.start", async () => {
		const { startGatewayDiscovery } = await import("./server-discovery-runtime-CLhNC0Yv.js");
...
async function startGatewayEarlyRuntime(params) {
	...
	if (!params.minimalTestGateway) {
		const [{ primeRemoteSkillsCache, setSkillsRemoteRegistry }, taskRegistryMaintenance] = await measureStartup(params.startupTrace, "runtime.early.lazy-runtime-imports", () => Promise.all([import("./skills-remote-C3RMF9e_.js"), import("./task-registry.maintenance-D7xGxcBi.js")]));
	...
	const skillsChangeUnsub = params.minimalTestGateway ? () => {} : await measureStartup(params.startupTrace, "runtime.early.skills-listener", async () => {
		const [{ registerSkillsChangeListener }, { refreshRemoteBinsForConnectedNodes }] = await Promise.all([import("./refresh-B8lSqW_B.js"), import("./skills-remote-C3RMF9e_.js")]);
	...
	const startMaintenance = async () => {
		...
		return await measureStartup(params.startupTrace, "post-ready.maintenance", async () => {
			const { startGatewayMaintenanceTimers } = await import("./server-maintenance-CAtB8gTR.js");
